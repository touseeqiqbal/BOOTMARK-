...
export default function WorkOrderForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let savedWorkOrder;
            if (isEditMode) {
                const res = await api.put(`/work-orders/${id}`, formData);
            ...
            if (formData.autoCreateInvoice && formData.status === 'completed') {
                try {
                    ...
                    const invoiceData = {
                        ...
                        total: total * 1.1,
                        status: 'pending',
                        notes: `Auto-generated from Work Order ${savedWorkOrder.workOrderNumber}`
                    };
                    ...
                    <div className="form-group" style={{ marginBottom: '24px', padding: '16px', background: '#eff6ff', borderRadius: '8px', border: '1px solid #bfdbfe' }}>
                        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', margin: 0 }}>
                            ...
                            <span style={{ fontWeight: '500', color: '#1e40af' }}>
                                <Receipt size={16} style={{ display: 'inline', marginRight: '4px', verticalAlign: 'middle' }} />
                                Auto-create invoice when work order is marked as completed
                            </span>
                        </label>
                        <p style={{ margin: '8px 0 0 26px', fontSize: '13px', color: '#6b7280' }}>
                            When enabled, an invoice will be automatically generated from this work order when its status is set to "completed". The invoice will include all services and materials.
                        </p>
                    </div>
