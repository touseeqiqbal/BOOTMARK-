...
app.use("/api/work-orders/form-settings", authRequired, workOrderFormSettingsRouter);
...
app.use("/api/contract-templates", authRequired, contractTemplatesRouter);
...
app.use("/api/workflows", authRequired, workflowsRouter); // NEW: Custom Workflows
app.use("/api/onboarding", authRequired, onboardingRouter); // NEW: User onboarding
app.use("/api/notifications", authRequired, notificationsRouter); // NEW: Notifications system
...
app.use((req, res, next) => {
  ...
  const appRoutes = [
    ...
    '/settings',
    '/client-portal',
    '/client',  // Client invitation/set-password pages
    '/profile'
  ];
