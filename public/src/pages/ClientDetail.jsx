...
export default function ClientDetail() {
    ...
    const confirmDeleteMessage = async () => {
        ...
        try {
            await api.delete(`/messages/${deleteMessageTarget}`)
            // Remove from local state
            setMessages(messages.filter(m => m.id !== deleteMessageTarget))
    ...
    const fetchClientData = async () => {
        try {
            ...
            try {
                const messagesRes = await api.get('/messages')
                // Safety check
                const clientMessages = Array.isArray(messagesRes.data)
                    ? messagesRes.data.filter(m => m.customerId === id)
                    : []
                setMessages(clientMessages)
            } catch (err) {
