...
router.post("/form/:shareKey/submit", async (req, res) => {
  try {
    ...
    try {
      const { customerName, customerEmail, customerPhone, customerAddress } = extractCustomerInfo(form, submissionData);
      console.log('[Public] Extracted customer info:', { customerName, customerEmail, customerPhone, customerAddress });
      ...
      if (hasValidName || customerEmail) {
        ...
        if (customerAddress && customerAddress.trim()) {
          try {
            const userId = form.userId;
            // Check if property already exists for this customer
            const allProperties = await getProperties();
            const existingProp = allProperties.find(p => p.customerId === customer.id && p.address === customerAddress);
            ...
            if (!existingProp) {
              const newProperty = {
                ...
                businessId: customer.businessId || null,
                createdAt: new Date().toISOString(),
                updatedAt: new Date().toISOString(),
                createdBy: userId,
                isDefault: true
              ...
              console.log(`[Public] Automatically created property for customer ${customer.id}`);
            }
          } catch (propError) {
            console.error("[Public] Failed to auto-create property:", propError);
          }
        }
      } else {
