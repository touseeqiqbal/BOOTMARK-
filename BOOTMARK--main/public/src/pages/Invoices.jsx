...
import { FileText, Plus, Edit, Trash2, Send, Download, Search, CheckCircle, XCircle, Clock, Upload, Square, CheckSquare } from 'lucide-react'
...
export default function Invoices() {
  ...
  const handleSendEmail = async (invoice) => {
    ...
    try {
      const response = await api.post(`/invoices/${invoice.id}/send`, { to: email })
  ...
  const handleExportPDF = async (invoice) => {
    try {
      ...
      doc.roundedRect(pageWidth - 160, 40, 110, 32, 16, 16, 'F')
      doc.setTextColor(...statusStyle.text)
      doc.setFontSize(12)
      doc.text((invoice.status || 'draft').toUpperCase(), pageWidth - 105, 61, { align: 'center' })
      ...
      if (branding.companyLogo) {
        ...
        doc.setFontSize(18)
        doc.text(branding.companyName, pageWidth - 40, 60, { align: 'right' })
      }
