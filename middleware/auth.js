          ...
          try {
            ...
            if (userDoc.exists) {
              ...
              if (!req.user.businessId && req.user.role === 'admin') {
                ...
                // Set flag to trigger onboarding flow in frontend
                req.user.requiresOnboarding = true;

                // Still allow authentication but flag for onboarding
                // Frontend should redirect to /onboarding if this flag is set
              }
            }
          } catch (firestoreError) {
